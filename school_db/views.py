...
def problem_one(request):
  ...
  gpa_over_three = Student.objects.all().filter(gpa__gt='3.0').order_by('-gpa')

  for student in gpa_over_three:

    print(f"First Name: {student.first_name} GPA: {student.gpa}")
...
def problem_two(request):
  ...
  hired_prior_2010 = Instructor.objects.filter(hire_date__lte="2010-01-01").order_by('hire_date')

  for instrutor in hired_prior_2010:

    print(f"Full Name: {instrutor.first_name} {instrutor.last_name}\nHire date: {instrutor.hire_date}")
...
def problem_four(request):
  ...
  course_counter = Course.objects.count()
  print(f"Courses count: {course_counter}")

  instructor_count = Instructor.objects.count()
  print(f"Instructor count: {instructor_count}")

  # course_count= Course.objects.
  return complete(request)
...
def problem_five(request):
  ...
  new_student = Student.objects.create(first_name = "Daniel", last_name = "Two", year = 2022, gpa = 3.0)
